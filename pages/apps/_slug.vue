<template>
  <div class="w-full flex flex-wrap py-10 bg-gray-100 invert" >
    ...
    <div class="px-0 md:px-6 lg:px-10 w-full flex flex-wrap justify-center items-center" v-lazy-container="{ selector: 'picture' }">
      <picture v-if="article.banner_url" class="min:h-10 min:w-10 w-full h-full self-center inline-block rounded-none md:rounded-lg shadow-lg">
        <source :srcSet="require(`~/static/${article.banner_url}?webp?png`)" type="image/png" />
        <img :data-src="require(`~/static/${article.banner_url}?webp`)"
            :data-loading="require(`~/static/${article.banner_url}?lqip`)"
            class="min:h-10 min:w-10 w-full h-full self-center inline-block rounded-none md:rounded-lg shadow-lg" :alt="article.title">
      </picture>
      <picture v-if="!article.banner_url" class="min:h-10 min:w-10 w-full h-full self-center inline-block rounded-none md:rounded-lg shadow-lg">
        <source :srcSet="require(`~/static/noimage.png?webp`)" type="image/webp" />
        <img :data-src="require(`~/static/noimage.png`)"
