    ...
    <div class="w-full flex flex-col md:flex-row flex-wrap items-stretch justify-center py-4 bg-gray-50" style="z-index:20;">
        <div class="px-0 md:px-6 lg:px-10 pb-10 flex flex-wrap relative items-start justify-center" style="z-index:20;">
            <div class="w-full h-16 flex flex-wrap items-start justify-center mb-4 px-4" style="z-index:20;">
                <span style="z-index:20;" class="font-medium text-4xl text-white" v-scroll-reveal="{ delay: 100, mobile: true, origin: 'top', easing: 'cubic-bezier(0.5, 0, 0, 1)', distance: '80px'}">Blog</span>
            </div>
            <div class="w-full max-w-6xl flex flex-wrap -mx-4 justify-center items-stretch">
                <div v-for="(blog, i) in blogs" :key="i" class="blog-card my-4 px-4 w-full md:w-1/2 lg:w-1/3 flex flex-wrap justify-between items-stretch select-none">
                    <div class="w-full rounded-lg duration-300 transition-all grid grid-rows-2 shadow-xl bg-gray-100">
                        <nuxt-link :to="`${blog.path}/`" class="focus:outline-none w-full block rounded-t-md md:rounded-t-lg h-full overflow-hidden object-cover" v-lazy-container="{ selector: 'img' }">
